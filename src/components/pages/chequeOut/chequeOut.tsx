...
export const ChequeOut = (props: IPropsHome) => {
  ...
  const handleChange = (setState: (param: string) => void) => (event: any) => {
  ...
  const signData = async (ref: string) => {
    ...
    const sign = await createIntegrityKey({
      ...
      SecretoIntegridad: process.env.NEXT_PUBLIC_INTEGRITY_KEY_WOMPI ?? "",
    });
  ...
  const validInput = (
    ...
    if (value.length < min || value.length > max) {
      return (
        "Debe tener mínimo de " + min + " y un máximo de " + max + " caracteres"
      );
    }
  ...
  useEffect(() => {
    ...
    // setNumbers(getStore(nameStore.NUMBERS, true));
  }, []);
    ...
    <Layout
        ...
        <div className={styles.column}>
          ...
          <form>
            <div>
              ...
              />
              {name.length > 0 && (
                <small>{validInput(name, "string", 100, 10)}</small>
              )}
            </div>
            <div>
              <input
            ...
            <div>
              ...
              />
              {email.length > 0 && (
                <small>{validInput(email, "email", 100, 5)}</small>
              )}
            </div>
          </form>
        </div>
            ...
            <div className={styles.card_container}>
              <div className={styles.card_container_data}>
                <div className={styles.data}>
                  ...
                  <p>{convertToMoney((dataRifa?.precio ?? 0) * numbers)}</p>
                </div>
                <div className={styles.data}>
                  <p>Subtotal</p>
                  <p>{convertToMoney((dataRifa?.precio ?? 0) * numbers)}</p>
              ...
              {/* <div className={styles.data_numbers}>
								<p>Tus números</p>
      ...
      <div className={styles.chequeOut_footer}>
        <form
          ...
          <input type="hidden" name="signature:integrity" value={signWompi} />
          <input
            type="hidden"
            name="redirect-url"
            value="https://cg79z20b-3000.uks1.devtunnels.ms/estado-compra"
          />
